...
void imguiBeginFrame(int32_t _mx, int32_t _my, uint8_t _button, int32_t _scroll, uint16_t _width, uint16_t _height, int _inputChar = -1, max::ViewId _view = 255);
void imguiEndFrame();
...
#define IMGUI_FLAGS_ALPHA_BLEND UINT8_C(0x01)
	...
	///
	inline ImTextureID toId(max::TextureHandle _handle, uint8_t _flags, uint8_t _mip)
	{
		union { struct { max::TextureHandle handle; uint8_t flags; uint8_t mip; } s; ImTextureID id; } tex;
	...
	// Helper function for passing max::TextureHandle to ImGui::Image.
	inline void Image(max::TextureHandle _handle
		, uint8_t _flags
		, uint8_t _mip
		, const ImVec2& _size
	...
	// Helper function for passing max::TextureHandle to ImGui::ImageButton.
	inline bool ImageButton(max::TextureHandle _handle
		, uint8_t _flags
		, uint8_t _mip
		, const ImVec2& _size
		, const ImVec2& _uv0     = ImVec2(0.0f, 0.0f)
