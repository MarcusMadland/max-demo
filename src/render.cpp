...
struct Uniforms
{
	void create()
	{
		...
		u_perDraw = max::createUniform("u_perdraw", max::UniformType::Vec4, 2);
	}
	...
	void submitPerDraw()
	{
		max::setUniform(u_perDraw, m_perDraw, 2);
	}
	...
	union
	{
		struct
		{
			union
			{
				...
				/* 0*/ struct { float m_m_invViewProj0[4]; };
				/* 1*/ struct { float m_m_invViewProj1[4]; };
				/* 2*/ struct { float m_m_invViewProj2[4]; };
				/* 3*/ struct { float m_m_invViewProj3[4]; };
			};
			/*4*/ struct { float m_lightDir[3],   m_unused10[1]; };
			/*5*/ struct { float m_lightCol[3],   m_unused11[1]; };
			/*6*/ struct { float m_volumeMin[3],  m_unused12[1]; };
			/*7*/ struct { float m_volumeMax[3],  m_unused13[1]; };
			/*8*/ struct { float m_volumeSize[3], m_unused14[1]; };
			/*9 COUNT*/
		};
...
struct Probes
{
	struct Probe { bx::Vec3 m_pos; };
	struct Dimensions { uint32_t x, y, z; };

	void create(const bx::Vec3& _pos, const Dimensions& _size, const float _spacing)
	{
		m_size = _size;
		m_num = m_size.x * m_size.y * m_size.z;
		...
		for (uint32_t x = 0; x < m_size.x; ++x)
		{
			for (uint32_t y = 0; y < m_size.y; ++y)
			{
				...
				{
					m_probes[index].m_pos = bx::Vec3(x * _spacing, y * _spacing, z * _spacing);
					m_probes[index].m_pos = bx::add(m_probes[index].m_pos, _pos);
					++index;
				}
			}
		}
	...
	const bx::Vec3 min()
	{
		return bx::Vec3(m_position[0], m_position[1], m_position[2]);
	}

	const bx::Vec3 max()
	{
		bx::Vec3 sizeVec = bx::Vec3((m_size.x - 1) * m_spacing,
			(m_size.y - 1) * m_spacing,
			(m_size.z - 1) * m_spacing);
		return bx::add(min(), sizeVec);
	}

	const bx::Vec3 size()
	{
		return bx::Vec3((float)m_size.x, (float)m_size.y, (float)m_size.z);
	}
...
// @todo Move this.
constexpr uint32_t kCubemapResolution = 48; 
...
struct GI
{
	void create(CommonResources* _common, max::ViewId _view)
	{
		...
		m_atlasWidth = (m_common->m_probes->m_size.x * m_common->m_probes->m_size.z) * kOctahedralResolution;
		m_atlasHeight = m_common->m_probes->m_size.y * kOctahedralResolution;
	...
	void render()
	{
		if (isValid(m_diffuseAtlas) && isValid(m_normalAtlas) && isValid(m_positionAtlas))
		{
			// Directional Light.
			float proj[16];
	...
	void precompute()
	{
		...
		for (uint32_t ii = 0; ii < m_common->m_probes->m_num; ++ii)
		{
			...
			for (uint8_t jj = 0; jj < CubemapSides::Count; ++jj)
			{
				...
				float proj[16];
				bx::mtxProj(proj, 90.0f, 1.0f, 0.1f, 1000.0f, max::getCaps()->homogeneousDepth);
				...
				max::setViewClear(m_viewIdOffline, MAX_CLEAR_COLOR | MAX_CLEAR_DEPTH, 0x7fb8ffff, 1.0f, 0);
			...
			uint32_t probeX = ii % m_common->m_probes->m_size.x;
			uint32_t probeZ = (ii / m_common->m_probes->m_size.x) % m_common->m_probes->m_size.z;
			uint32_t probeY = ii / (m_common->m_probes->m_size.x * m_common->m_probes->m_size.z);

			uint32_t atlasPosX = probeX * kOctahedralResolution + probeZ * kOctahedralResolution * m_common->m_probes->m_size.x;
			uint32_t atlasPosY = probeY * kOctahedralResolution;

			max::blit(m_viewIdOffline, m_diffuseAtlas, atlasPosX, atlasPosY, diffuseOct);
			max::blit(m_viewIdOffline, m_normalAtlas, atlasPosX, atlasPosY, normalOct);
			max::blit(m_viewIdOffline, m_positionAtlas, atlasPosX, atlasPosY, positionOct);
	...
	max::UniformHandle s_normal; //!< Diffuse uniform
	max::UniformHandle s_position; //!< Diffuse uniform
...
struct GBuffer
{
	...
	void render()
	{
		...
		max::setViewClear(m_view, MAX_CLEAR_COLOR | MAX_CLEAR_DEPTH, 0x7fb8ffff, 1.0f, 0);
...
struct RenderSystem
{
	void create(RenderSettings* _settings)
	{
		...
		m_uniforms.create();
		m_material.create();
		m_probes.create({ -4.0f, 1.0f, -4.0f }, { 5, 5, 5 }, 2.0f);
